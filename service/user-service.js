...
class UserService {
  async rigistration(email, password, name, lastname) {
    try {
      ...
      if (IsEmailRegistered.state == true) {
        ...
        throw ApiError.BadRequest(
          "ошибка при вводе формы или email уже используется"
        );
      } else {
        const Addresult = await addUser(email, password, name, lastname);

        if (Addresult) {
          await mailService.sendActivationMail(
            email,
            `${process.env.API_URL}/server/api/activate/${Addresult.ActivationLink}`
          );
  ...
  async sendActivationMail(email) {
    ...
    if (!user.SysLevel) {
      await mailService.sendActivationMail(
        email,
        `${process.env.API_URL}/server/api/activate/${user.ActivationLink}`
      );
      return user;
    } else {
